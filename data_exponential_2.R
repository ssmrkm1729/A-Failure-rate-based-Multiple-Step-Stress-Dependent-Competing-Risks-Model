...
opt_C=C_param(1, opt_p,  m, t, failure_count, S , tau, r) + C_param(2, opt_p,  m, t, failure_count, S , tau, r) +C_param(3, opt_p,  m, t, failure_count, S , tau, r)
...
contour_gg <- ggplot(contour_dataframe, aes(x = P_values)) +
    ...
    scale_color_manual(
    name = NULL,
    values = c("Profile log-likelihood" = "black")
  ) +
...
indicator1
indicator2
prof_log_likelihood(param[1],  m, t, failure_count, S , tau, r)
...
MLE_data <- data.frame(Parameters= c("P","$c_0$","$c_1$","$c_2$"))
MLE_data$MLE = param
