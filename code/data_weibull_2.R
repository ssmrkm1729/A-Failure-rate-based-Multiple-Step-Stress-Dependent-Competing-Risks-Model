...
D_F <- function(t, lamb, alpha) {
  return(1 - exp(-lamb * (t^alpha)))
}

ln_f_not <- function(t, alpha) {
  return(log(alpha) + (alpha - 1) * log(t) - (t^alpha))
}
...
C_param<-function(b, P, alpha, m, t, failure_count, S , tau, r)
{
  ...
  for (l in c(1:k))
  {
    ...
    a=(sum(as.integer(failure_count[[i]]))+r[[i]])*log(1-F_not(tau[l+1], alpha)) * ((S[l]^P)-(S[l+1]^P))
    t2=t2+a
  ...
  
{
  ...
  for (l in c(1:k))
  {
    ...
    a=C*(sum(as.integer(failure_count[[i]]))+r[[i]])*log(1-F_not(tau[l+1], alpha)) * ((S[l]^P)-(S[l+1]^P))
    t3=t3+a
...
alpha_values <- seq(from = 0.001, to = 6, by = 0.05)
...
opt_C=C_param(1, opt_p, opt_alpha, m, t, failure_count, S , tau, r) + C_param(2, opt_p, opt_alpha, m, t, failure_count, S , tau, r) +C_param(3, opt_p, opt_alpha, m, t, failure_count, S , tau, r)
...
source_path <- dirname(rstudioapi::getActiveDocumentContext()$path)
setwd(source_path)
orca(fig_2d, file = "contour_frm_weibull.pdf", format = "pdf", width = 1600, height = 1200, scale = 3)
...
result_1 <- optim(par = initial_params, fn = objective_func,method = "L-BFGS-B",lower = lower_bound,upper = upper_bound, hessian = T)
...
indicator1= param-qnorm(p=.05/2, lower.tail=FALSE)*((variance^0.5))
indicator2= param+qnorm(p=.05/2, lower.tail=FALSE)*((variance^0.5))
...
min_dataframe= data.frame(x1_emp_min = x1_emp_min, 
...
fig_minimum_gg <- ggplot(min_dataframe, aes(x = x1_emp_min)) +
  ...
  geom_ribbon(aes(ymin = y1_emp_lower_conf_min, ymax = y1_emp_upper_conf_min, fill = "Empirical Confidence Interval"), 
              alpha = 0.5) +
...
margin_1_dataframe= data.frame(x1_emp_margin_1 = x1_emp_margin_1, 
...
fig_margin_1_gg <- ggplot(margin_1_dataframe, aes(x = x1_emp_margin_1)) +
  ...
  geom_ribbon(aes(ymin = y1_emp_lower_conf_margin_1, ymax = y1_emp_upper_conf_margin_1, fill = "Empirical Confidence Interval"), 
              alpha = 0.5) +
...
margin_2_dataframe= data.frame(x1_emp_margin_2 = x1_emp_margin_2, 
...
fig_margin_2_gg <- ggplot(margin_2_dataframe, aes(x = x1_emp_margin_2)) +
  ...
  geom_ribbon(aes(ymin = y1_emp_lower_conf_margin_2, ymax = y1_emp_upper_conf_margin_2, fill = "Empirical Confidence Interval"), 
              alpha = 0.5) +
...
indicator1
indicator2
prof_log_likelihood(param[1], param[2], m, t, failure_count, S , tau, r)
