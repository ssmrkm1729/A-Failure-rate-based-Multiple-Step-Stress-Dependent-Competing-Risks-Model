...
# Failure times by stage
t = list(c(stress_data_stage_1$time),
...
failure_count = list(c(n_10, n_11, n_12),
                     c(n_20, n_21, n_22),
                     c(n_30, n_31, n_32))
r = list(r_1, r_2, r_3)
...
# Baseline distribution: CDF
F_not <- function(t, alpha) {
  return(1 - (1+t**alpha)**(-1))
}

# Log of baseline density
ln_f_not <- function(t, alpha) {
  return(log(alpha) + (alpha - 1) * log(t) - 2*log(1+t**alpha))
}

# Log of baseline survival
ln_S_not <- function(t, alpha) {
  return(-log(1+t**alpha))
}

# Stress-dependent failure rate
...
# Function for C parameter estimation
...
#  search grid for P and alpha
P_values <- seq(from = 0.001, to = 20, by = 0.05) 
alpha_values <- seq(from = 0.001, to = 10, by = 0.05)
...
#  grid point with maximum log-likelihood
max_value <- max(z)
max_position <- which(z == max_value, arr.ind = TRUE)

# Optimal P and alpha from  search
opt_p = P_values[max_position[1]]
opt_alpha = alpha_values[max_position[2]]

#  optimal total C and stress-dependent lambdas
...
fig_2d <- add_trace(
  ...
  layout(
    xaxis = list(
      title = list(text = "\u03B1", font = list(size = 48, color = "black")),
      tickfont = list(size = 40, color = "black")
    ),
    yaxis = list(
      title = list(text = "P", font = list(size = 48, color = "black")),
      tickfont = list(size = 40, color = "black")
    ),
    margin = list(b = 80)
  )
...
# Save 2D contour 
orca(fig_2d, file = "contour_frm_burr.pdf", format = "pdf", width = 1600, height = 1200, scale = 3)
...
#  
fig_3d <- fig_3d %>% layout(scene = list(
...
# Objective function for optimization (negative profile log-likelihood)
...
#  estimated C parameters using estimated P and alpha
...
# Function for the CDF of the minimum failure time
...
# Empirical CDF for minimum failure time
...
y1_emp_min = c()
y1_emp_lower_conf_min = c()
y1_emp_upper_conf_min = c()
for(i in x1_emp_min) {
...
fig_minimum_gg <- ggplot(min_dataframe, aes(x = x1_emp_min)) +
  ...
  geom_ribbon(aes(ymin = y1_emp_lower_conf_min, ymax = y1_emp_upper_conf_min, 
                  fill = "Empirical Confidence Interval"), alpha = 0.5) +
  ...
  scale_color_manual(name = NULL, values = c("Empirical Probability" = "blue", 
                                             "Theoretical Probability" = "red")) +
...
# Function for marginal CDF for cause 1
...
y1_emp_margin_1 = c()
y1_emp_lower_conf_margin_1 = c()
y1_emp_upper_conf_margin_1 = c()

#  empirical survival and confidence limits
...
#  theoretical CDF for marginal 1
y1_theo_margin_1 = c()
for(i in x1_emp_margin_1) {
...
# Plot empirical vs theoretical marginal 1 CDF
fig_margin_1_gg <- ggplot(margin_1_dataframe, aes(x = x1_emp_margin_1)) +
  ...
  geom_ribbon(aes(ymin = y1_emp_lower_conf_margin_1, ymax = y1_emp_upper_conf_margin_1, 
                  fill = "Empirical Confidence Interval"), alpha = 0.5) +
...
fig_margin_1_gg  # Display plot




# Function for marginal CDF for cause 2
...
# Empirical CDF for marginal 2 (cause 2)
...
#  vectors for empirical survival & confidence limits
y1_emp_margin_2 = c()
y1_emp_lower_conf_margin_2 = c()
y1_emp_upper_conf_margin_2 = c()
...
# Plot empirical vs theoretical marginal 2 CDF
...
fig_margin_2_gg <- ggplot(margin_2_dataframe, aes(x = x1_emp_margin_2)) +
  ...
  geom_ribbon(aes(ymin = y1_emp_lower_conf_margin_2, ymax = y1_emp_upper_conf_margin_2, 
                  fill = "Empirical Confidence Interval"), alpha = 0.5) +
...
#  model log-likelihood & AIC table
...
#  MLE and model info to Excel
write.xlsx(list(MLE_data = MLE_data, model_llh = model_llh), file = file_name, rowNames = FALSE)


#  PDF file names for figures
